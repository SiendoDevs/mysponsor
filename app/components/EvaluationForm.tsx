...
const EvaluationForm: React.FC<EvaluationFormProps> = ({ onFormSubmit, initialData }) => {
  const [firstName, setFirstName] = useState(initialData.firstName);
  const [lastName, setLastName] = useState(initialData.lastName);
  const [nickName, setNickName] = useState(initialData.nickName);
  const [experience, setExperience] = useState(initialData.experience ? String(initialData.experience) : "0");
  const [desired_budget, setBudget] = useState(String(initialData.desired_budget).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
  ...
  const validate = () => {
    ...
    if (!desired_budget || isNaN(Number(desired_budget.replace(/\,/g, ""))) || Number(desired_budget.replace(/\,/g, "")) < 0) {
      ...
      <Card className="max-w-lg w-full">
        <form onSubmit={handleSubmit}>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="experience">Experiencia en Karting ({experience} años)</Label>
              <Slider
                id="experience"
                min={0}
                max={20}
                ...
                className="py-4"
              />
              {errors.experience && <p className="text-destructive text-sm">{errors.experience}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">ARS</span>
              </div>
              <p className="text-sm text-muted-foreground">Sugerencia: Entre $100.000 y $500.000 ARS</p>
              {errors.desired_budget && <p className="text-destructive text-sm">{errors.desired_budget}</p>}
            </div>
          </CardContent>
